...
| 等级 | 含义 | 判定信号 | 前置动作（必须给） |
|---|---|---|---|
| **极高危** | 不可逆删除 / 覆盖，**或使系统（或服务）不可逆失效** | `rm -rf`、格式化、`DROP`、`TRUNCATE`、无 WHERE 的 `DELETE`/`UPDATE`、`git reset --hard`、**`chmod`/`chown` 递归作用于根目录**、`dd if= of=/dev/*`、`find -delete`、`git clean -fd` | 先备份 / 先 SELECT 看行数 / 先 `--dry-run`；**禁止**未经确认产出 |
| **危险** | 批量修改 / 覆盖，**或覆盖已存在文件致其原内容不可逆丢失** | `mv` 批量、`sed -i`、`git push --force`、无确认的重命名脚本、**`:> f` / `> f`（截断）、`cp a b`（b 已存在）、`mysqldump > 已有备份`** | 先 `ls`/打印预览确认范围；覆盖前先 `cp` 备份 |
| **谨慎** | 影响可控但有副作用（**不含「覆盖已有内容」**） | 有 WHERE 的 `UPDATE`、**写入不存在的文件（新建）或追加**、**非递归 / 目标明确的** `chmod`·`chown`、`kill` 指定 PID | 说明影响对象，确认目标 |
| **安全** | 只读 | `grep`、`cat`、`SELECT`、`ls`、正则匹配、`wc` | 无需前置 |

**两处易错的分界线（一定要按这两条判，不要凭命令族直觉）：**

1. **是否「覆盖已有内容」**：把 `>` / `:>` / `cp` / `mysqldump >` 写到**已存在**的文件上，
   旧内容**不可逆丢失** → 一律 **危险**（不是「谨慎」）。写到不存在的文件（新建）或追加 → 谨慎。
2. **是否「递归作用于根/系统级路径」**：`chmod -R` / `chown -R` 作用到 `/`、`/etc`、`/usr` 这类路径，
   会让系统或服务**不可逆失效**（通常只能重装恢复）→ 一律 **极高危**（不是「谨慎」）。
   命令族（`chmod` 本身）不决定等级，**作用范围**才决定等级。
...
| `chmod -R 777 /` | **极高危** | 仅对目标目录 `chmod`，不用 `-R` 递归到根 | 递归改系统级权限，SSH/服务可能直接失效，恢复需重装 |
...
| `chown -R user /` | **极高危** | 只对目标目录 `chown`，不用 `-R` 到根 | 同 `chmod -R`：递归改属主，系统不可逆失效 |
| `:> file` / `> file` | **危险** | 先 `cp file file.bak` 备份 | 截断清空文件，原内容不可逆丢失 |
| `cp a b`（b 已存在） | **危险** | `cp -i a b` 交互确认覆盖 | 覆盖已存在文件 → b 的原内容不可逆丢失 |
| `rsync --delete src dst` | 危险 | 先 `rsync -n --delete` 干跑看差异 | 删目标多余文件 |
| `pkill -f xxx` | 危险 | 先 `pgrep -f xxx` 看匹配进程 | 误杀其他进程 |
| `mysqldump ... > out.sql`（out 已存在） | **危险** | 先确认 out 路径或改名（建议带时间戳） | 覆盖已有备份 → 旧备份不可逆丢失，且往往在需要它的时候才发现 |
